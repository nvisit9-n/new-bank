...
export const QuizScreen: React.FC = () => {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className="p-5 sm:p-6 rounded-3xl bg-[#0B192C] text-white shadow-xl border border-slate-700/80 space-y-4">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-700/80 pb-4">
          ...
          <div className="flex items-center gap-3 shrink-0 bg-slate-800/90 px-4 py-2 rounded-2xl border border-slate-700">
            <Target className="w-6 h-6 text-emerald-400" />
            <div>
              <span className="text-[10px] text-slate-400 font-bold block uppercase tracking-wider">१० वटै मोड्युल</span>
            ...
            return (
              <button
                ...
                className={`p-3 rounded-2xl border text-left transition-all cursor-pointer group ${
                  isSelected
                    ? 'bg-emerald-950/80 border-emerald-500 shadow-md ring-1 ring-emerald-500/40 scale-[1.02]'
                    : 'bg-slate-900/90 border-slate-800 hover:border-emerald-500/50 hover:bg-slate-800/80'
                }`}
              >
                <div className="flex items-center justify-between mb-1">
                  <span className="text-[10px] font-bold text-slate-400">#{idx + 1}</span>
                  <span className="text-[10px] font-extrabold px-1.5 py-0.5 rounded-full bg-emerald-950/90 text-emerald-300 border border-emerald-500/40 group-hover:bg-emerald-500 group-hover:text-slate-950 transition">
                    ▶ सुरु ({selectedCount})
                  </span>
                </div>
                ...
                <p className="text-[10px] text-slate-400 line-clamp-1 mt-0.5">{mod.subTopics.slice(0, 2).map(s => s.nameNepali).join(', ')}</p>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800 text-[11px] text-slate-300 font-medium">
          <span className="text-amber-400 font-bold">📌 प्रमाणित तथ्य नियम:</span>
          <span className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-200">सिन्धुपाल्चोक = भोटेकोशी नदी (अरनिको/तातोपानी)</span>
          <span className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-200">रसुवा = त्रिशूली नदी (पासाङ ल्हामु/रसुवागढी)</span>
          <span className="px-2 py-0.5 rounded bg-slate-800 border border-slate-700 text-slate-200">Policy Rate: ५.५% | Bank Rate: ६.५% | CRR: ४.०% | SLR: १२.०%</span>
